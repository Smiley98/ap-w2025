...
class Player
{
	// Hot-take -- never write constructors. They make your code needlessly complicated (although the lecture slides will tell you otherwise).
public:
	// Default constructor. Normally generated by the compiler, but we need to make on manually if we have 1+ custom constructors
	Player() {}

	// The moment you define a custom constructor, you must also define a default constructor
	Player(float health) { mHealth = health; }

	Player(std::string name, float health) { mName = name; mHealth = health; }

	// Destructor (can only have 1). Runs when an object is destroyed
	~Player()
	{
		printf("Destroyed player %s with health %f.\n", mName.c_str(), mHealth);
	}

private:
	// The 'm' in "mHealth" stands for private-class-[M]ember
...
{
	// "Default" constructor runs in the absense of any custom initialization
	Player player1;

	// Create a player with more health if they're playing on "easy-mode"
	Player recruitDifficulty(200.0f);

	// Run our custom constructor to handle the case that Snoop Dog is playing our game
	Player snoopDog("D-O-Double-G", 420.0f);
}
